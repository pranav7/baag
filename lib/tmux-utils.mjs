...
// Read worktree setup commands from config
async function getWorktreeSetupCommands(repoPath) {
  try {
    const configPath = path.join(repoPath, '.baag', 'config.json')
    ...
    // Object format with platform-specific
...
export async function createTmuxSession(worktreeName, worktreePath, horizontalSplit = false, hookEnv = null) {
  ...
  try {
    ...
    if (configuredAI) {
      ...
      // Check if it's a direct command or might be an alias
      try {
        await $`which ${configuredAI}`.quiet()
        // It's a direct command
        aiCmd = configuredAI
      } catch {
        // Might be an alias, try loading shell config
        const shellConfig = detectShellConfig()
        try {
          await fs.access(shellConfig)
          aiCmd = `source ${shellConfig} && ${configuredAI}`
          printInfo(`Loading shell config for ${configuredAI} alias`)
        } catch {
          // No shell config found, try the command anyway
          aiCmd = configuredAI
          printInfo(`Attempting to run ${configuredAI} directly`)
        }
      }
    } else {
    ...
    // Terminal pane: Execute worktree setup commands if configured
    const setupCommands = await getWorktreeSetupCommands(mainRepoPath)

    if (setupCommands && setupCommands.length > 0) {
      printInfo(`Running worktree setup in terminal pane (${setupCommands.length} command(s))...`)

      // Set environment variables for setup
      if (hookEnv) {
        for (const [key, value] of Object.entries(hookEnv)) {
          await $`tmux send-keys -t "${sessionName}:${windowIndex}.${terminalPane}" "export ${key}='${value}'" Enter`
        }
      }

      // Show setup header in terminal pane
      await $`tmux send-keys -t "${sessionName}:${windowIndex}.${terminalPane}" "echo '=== Worktree Setup ==='" Enter`

      // Execute each setup command sequentially
      for (let i = 0; i < setupCommands.length; i++) {
        const command = setupCommands[i]
        printInfo(`  [${i + 1}/${setupCommands.length}] ${command}`)

        // Send command to terminal pane
        await $`tmux send-keys -t "${sessionName}:${windowIndex}.${terminalPane}" "${command}" Enter`

        // Wait a moment between commands (optional, can adjust)
        await sleep(500)
      }

      // Show completion message
      await $`tmux send-keys -t "${sessionName}:${windowIndex}.${terminalPane}" "echo '=== Setup Complete ==='" Enter`

      printSuccess('Worktree setup commands queued in terminal pane')
    }
