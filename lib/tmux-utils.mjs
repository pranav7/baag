...
export async function createTmuxSession(worktreeName, worktreePath, horizontalSplit = false) {
  ...
  try {
    ...
    // Left pane: Start configured AI agent or Claude as fallback
    ...
    // Send the command to the first pane (AI agent)
    await $`tmux send-keys -t ${sessionName}:${windowIndex}.0 '${aiCmd}' Enter`
    ...
    await setWorktreeConfig(worktreeName, 'ai-pane', '0') // AI agent is always in pane 0
    ...
    if (horizontalSplit) {
      console.log(`Top pane: ${colors.boldCyan(aiName)} | Bottom pane: ${colors.boldGreen('Terminal')}`)
    } else {
      console.log(`Left pane: ${colors.boldCyan(aiName)} | Right pane: ${colors.boldGreen('Terminal')}`)
    }
