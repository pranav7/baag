...
import os from 'os'
import { printInfo, printSuccess, printWarning, formatBranch, formatCommand, colors } from './colors.mjs'
  ...
  try {
    ...
    if (horizontalSplit) {
      ...
      console.log(`Left pane: ${colors.boldCyan('Claude')} | Right pane: ${colors.boldGreen('Terminal')}`)
        ...
        // Try to attach to session, but handle non-interactive execution gracefully
    try {
      await $`tmux attach-session -t ${sessionName}`
    } catch (error) {
      if (error.message.includes('not a terminal')) {
        printInfo('Session created successfully - attach manually with:')
