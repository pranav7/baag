...
import { getAIAgent, isCommandAvailable, getServerCommand, getSessionHooks } from './config-utils.mjs'
...
export async function createTmuxSession(worktreeName, worktreePath, horizontalSplit = false, port = null, hookEnv = null) {
  ...
  try {
    ...
    // Terminal pane: Execute onStart hooks if provided
    if (hookEnv) {
      const hooks = await getSessionHooks(mainRepoPath)
      const onStartHooks = hooks.onStart || []
      
      if (onStartHooks.length > 0) {
        printInfo(`Running onStart hooks in terminal pane...`)
        ...
        for (const command of onStartHooks) {
          ...
          await $`tmux send-keys -t "${sessionName}:${windowIndex}.${terminalPane}" "${processedCommand}" Enter`
        }
