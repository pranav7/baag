...
async function isCommandAvailable(command) {
  ...
  try {
    ...
    try {
      ...
      // Try with shell config for aliases
      ...
      if (shellConfig) {
        try {
          // Check if the alias exists in shell config
          const result = await $`bash -c "source ${shellConfig} 2>/dev/null && command -v ${command}"`.quiet()
...
async function runConfigWizard() {
  ...
  try {
    ...
    const aiOptions = [
      ...
      { label: 'Custom (specify command or alias)', value: 'custom' }
    ]

    let aiAgent = await p.select({
      message: 'Which AI agent would you like to use by default?',
      options: aiOptions,
      initialValue: config.aiAgent
    })
    
    // Handle custom AI agent
    if (aiAgent === 'custom') {
      aiAgent = await p.text({
        message: 'Enter your custom AI command or alias (e.g., claudex):',
        placeholder: 'claudex',
        validate: async (value) => {
          if (!value.trim()) return 'Command cannot be empty'
          const available = await isCommandAvailable(value.trim())
          if (!available) {
            return `Warning: '${value}' not found. Make sure it's installed or aliased.`
          }
        }
      })
