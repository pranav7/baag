...
// Execute worktree setup commands sequentially
async function executeWorktreeSetup(worktreePath, baseBranch) {
  const commands = await getWorktreeSetupCommands()

  if (!commands || !Array.isArray(commands) || commands.length === 0) {
    return
  }

  printHeader('Running Worktree Setup')
  printInfo(`Found ${commands.length} setup command(s) in config`)

  const mainRepoDir = await getMainRepoDir()

  // Set up environment variables for setup commands
  const setupEnv = {
    ...process.env,
    BAAG_ROOT_PATH: mainRepoDir,
    BAAG_WORKTREE_PATH: worktreePath,
    BAAG_BASE_BRANCH: baseBranch
  }
  ...
  try {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      console.log(`\n${colors.dim(`[${i + 1}/${commands.length}]`)} ${formatCommand(command)}`)

      try {
        // Execute via shell to properly handle complex commands
        await $({ env: setupEnv })`sh -c ${command}`
        printSuccess(`Command ${i + 1} completed`)
      } catch (error) {
        printError(`Command ${i + 1} failed: ${command}`)
        console.error(error.message)
        throw error
      }
    }

    printSuccess('Worktree setup completed successfully')
  } finally {
...
export async function startWorktree(args) {
  ...
  try {
    ...
    // Execute worktree setup commands if configured
    await executeWorktreeSetup(worktreePath, currentBaseBranch)
