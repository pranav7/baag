...
import { getBaseBranch, getCodeEditor, openCodeEditor, getAIAgent, getBranchPrefix, getServerCommand } from './config-utils.mjs'
import { assignPort, releasePort, getWorktreePort } from './port-utils.mjs'
import { executeStartHooks, executeStopHooks } from './hooks-utils.mjs'
...
export async function startWorktree(args) {
  ...
  if (!nonInteractive) {
    ...
    if (!baseBranchSpecified && !responses.baseBranch) {
      ...
      // Remove duplicates and sort with main branch context
      const uniqueBranches = [...new Set(availableBranches)].sort((a, b) => {
        if (a === mainBranch) return -1
        if (b === mainBranch) return 1
        return a.localeCompare(b)
      })

      // Create a helpful message with available branches
      const branchList = uniqueBranches.slice(0, 8).join(', ') +
        (uniqueBranches.length > 8 ? `, and ${uniqueBranches.length - 8} more...` : '')

      const selectedBaseBranch = await p.text({
        message: 'Enter base branch name:',
        placeholder: mainBranch,
        defaultValue: mainBranch,
        hint: `Available: ${branchList}`,
        validate: (value) => {
          if (!value || value.trim() === '') {
          ...
          // Warn if branch doesn't exist locally (but don't block it)
          if (!uniqueBranches.includes(trimmedValue)) {
            // This will create a new branch or use a remote branch
            return undefined
          }
  ...
  try {
    ...
    // Assign port if server command is configured
    let assignedPort = null
    const mainRepoPath = await getMainRepoDir()
    const serverCommand = await getServerCommand(mainRepoPath)
    if (serverCommand) {
      try {
        assignedPort = await assignPort(name)
        printSuccess(`Assigned port ${assignedPort} for development server`)
      } catch (error) {
        printWarning(`Failed to assign port: ${error.message}`)
      }
    }
    ...
    if (assignedPort) {
      hookEnv.BAAG_PORT = assignedPort
    }
    ...
    if (await checkTmuxClaude()) {
      ...
      const tmuxSuccess = await createTmuxSession(name, worktreePath, horizontalSplit, assignedPort, hookEnv)
      ...
      // Execute hooks when tmux is not available
      await executeStartHooks(name, hookEnv, worktreePath)
...
export async function resumeWorktree(args) {
  ...
  if (worktreeExistsAlready) {
    ...
    if (await checkTmuxClaude()) {
      ...
      const aiName = configuredAI ? configuredAI.charAt(0).toUpperCase() + configuredAI.slice(1) : 'AI agent'
      printInfo(`Both tmux and ${aiName} detected - creating integrated development environment`)
      // Get existing port assignment if available
      const existingPort = await getWorktreePort(name)
      await createTmuxSession(name, worktreePath, false, existingPort) // Default to vertical split
    } else {
...
async function startWorktreeForResume(args) {
  ...
  try {
    ...
    if (await checkTmuxClaude()) {
      ...
      const aiName = configuredAI ? configuredAI.charAt(0).toUpperCase() + configuredAI.slice(1) : 'AI agent'
      printInfo(`Both tmux and ${aiName} detected - creating integrated development environment`)
      // For resume create command, assign port if server configured but no existing port
      let resumePort = null
      const resumeMainRepoPath = await getMainRepoDir()
      const serverCommand = await getServerCommand(resumeMainRepoPath)
      if (serverCommand) {
        try {
          resumePort = await assignPort(name)
          printSuccess(`Assigned port ${resumePort} for development server`)
        } catch (error) {
          printWarning(`Failed to assign port: ${error.message}`)
        }
      }
      await createTmuxSession(name, worktreePath, false, resumePort) // Default to vertical split for resume
    } else {
      console.log(`Changing directory to: ${formatPath(worktreePath)}`)
      process.chdir(worktreePath)
      printInfo('Worktree is ready! You can now start working.')
...
export async function stopWorktree(args) {
  ...
  // Execute stop hooks first
  const hookEnv = { BAAG_WORKTREE: name }
  const port = await getWorktreePort(name)
  if (port) {
    hookEnv.BAAG_PORT = port
  }
  await executeStopHooks(name, hookEnv)
