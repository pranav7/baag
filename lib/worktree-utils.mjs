...
export async function startWorktree(args) {
  ...
  if (!nonInteractive) {
    ...
    if (!baseBranchSpecified && !responses.baseBranch) {
      ...
      // Sort branches with main branch first, then alphabetically
      ...
      // Create options with main branch highlighted
      const branchOptions = sortedBranches.map(branch => ({
        value: branch,
        label: branch === mainBranch ? `${branch} (main branch)` : branch,
        hint: branch === mainBranch ? 'Default base branch' : undefined
      }))

      // Add option to type custom branch name
      branchOptions.push({
        value: '__custom__',
        label: 'Type a different branch name...',
        hint: 'Enter any branch name manually'
      })

      const selectedBaseBranch = await p.select({
      ...
      if (selectedBaseBranch === '__custom__') {
