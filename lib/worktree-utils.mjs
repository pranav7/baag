...
import { getBaseBranch, getCodeEditor, openCodeEditor, getAIAgent } from './config-utils.mjs'
...
export async function startWorktree(args) {
  ...
  if (baseBranchSpecified) {
    ...
    // Use configured default base branch if available
    ...
    } else if (configuredBaseBranch) {
      printWarning(`Configured base branch '${formatBranch(configuredBaseBranch)}' does not exist, falling back to current branch`)
  ...
  try {
    ...
    if (await checkTmuxClaude()) {
      ...
      // Try to open the configured code editor
      const editorOpened = await openCodeEditor('.')
      if (editorOpened) {
