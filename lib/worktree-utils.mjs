...
async function executeWorktreeSetup(worktreePath, mainRepoPath) {
  ...
  printHeader('Worktree Setup')
  printInfo(`Running ${setupCommands.length} setup command(s) in worktree...`)
  console.log()
  ...
  try {
    ...
    // Execute each command sequentially with full output visibility
    for (let i = 0; i < setupCommands.length; i++) {
      const command = setupCommands[i]
      console.log(`${colors.cyan(`[${i + 1}/${setupCommands.length}]`)} ${colors.bold(command)}`)
      console.log()

      try {
        // Use child_process for proper shell execution with real-time output
        const { spawnSync } = await import('child_process')
        const result = spawnSync(command, {
          cwd: worktreePath,
          shell: true,
          stdio: 'inherit', // Show output in real-time to user
          encoding: 'utf8'
        })
        ...
        if (result.status === 0) {
          ...
          console.log()
          printError(`✗ Command failed with exit code ${result.status}`)
          printWarning('Continuing with remaining setup commands...')
        }
      } catch (error) {
        console.log()
    ...
    printSuccess('Worktree setup completed')
    console.log()
  } finally {
...
export async function startWorktree(args) {
  ...
  try {
    ...
    // Run worktree setup commands
    const mainRepoPath = await getMainRepoDir()
    await executeWorktreeSetup(worktreePath, mainRepoPath)
...
export async function submitWorktree(args) {
  ...
  // Skip PR creation unless --pr flag is set
  if (!createPr) {
    printSuccess('Branch pushed successfully!')
    printInfo('Create a PR with: baag finish --pr "PR title"')
  ...
  try {
    ...
    const ghArgs = ['pr', 'create', '--base', baseBranch, '--head', currentBranch, '--title', prTitle, '--body', '']
...
export async function cleanupWorktrees() {
  ...
  // First, prune git's worktree list to remove references to deleted worktrees
  try {
    printInfo('Pruning git worktree list...')
    await $`git worktree prune`
    printSuccess('Git worktree list pruned')
  } catch (error) {
    printWarning('Could not prune git worktree list')
  }
