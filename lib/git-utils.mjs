...
export function getWorktreesDir() {
  ...
  if (isInWorktree()) {
    // Find the .baag directory by going up from current path
    let currentPath = process.cwd()
    while (currentPath !== path.dirname(currentPath)) {
      if (path.basename(currentPath) === '.baag') {
        return currentPath
      }
      currentPath = path.dirname(currentPath)
    }

    // Fallback: assume we're in a subdirectory of .baag
    const parentDir = path.dirname(process.cwd())
    if (path.basename(parentDir) === '.baag') {
      return parentDir
    }
  }

  return path.join(mainRepo, '.baag')
}

export function isInWorktree() {
  const currentPath = process.cwd()
  return currentPath.includes('/.baag/')
}
