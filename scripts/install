...
// Setup .baag directory and .gitignore for projects
  ...
  try {
    ...
    if (!gitignoreContent.includes('.baag')) {
      const newContent = gitignoreExists
        ? gitignoreContent + '\n# Baag worktree directory\n.baag/\n'
        : '# Baag worktree directory\n.baag/\n'

      await fs.writeFile(gitignorePath, newContent)
      printSuccess("Added .baag to .gitignore")
    } else {
    ...
    if (error.stderr && error.stderr.includes('not a git repository')) {
      ...
      printInfo("Run 'baag start' from within a git repository to create .baag directory")
    } else {
      printWarning(`Could not setup project files: ${error.message}`)
    }
  }
}
